...
type Topic struct {
        ...
        startChan         chan int // Topic 创建完成调用Start方法, 会通知此channel, 消息处理方法监听到此Channel信息后开始处理消息
        exitChan          chan int // 调用Topic退出方法, 通知此channel, 消息处理方法监听到此Channel 信息后开始停止处理
        channelUpdateChan chan int // Channel 更新时, 通过此channel 通知消息处理方法更新处理的Channel列表
...
func NewTopic(topicName string, ctx *context, deleteCallback func(*Topic)) *Topic {
        ...
        // 临时topic 在memoryMsgChan 满了之后丢弃新到msg
...
func (t *Topic) put(m *Message) error {
        ...
        default:
                // 写入backend
                b := bufferPoolGet() // sync.Pool,  复用对象
...
func (t *Topic) messagePump() {
        ...
        for {
                select {
                case msg = <-memoryMsgChan: // 优先内存中收取msg
                case buf = <-backendChan: // backend中收取msg
                        msg, err = decodeMessage(buf)
                        if err != nil {
                                t.ctx.nsqd.logf(LOG_ERROR, "failed to decode message - %s", err)
                                continue
                        }
                case <-t.channelUpdateChan: // channel update
                        chans = chans[:0]
                        t.RLock()
                        for _, c := range t.channelMap {
                                chans = append(chans, c)
