...
func (p *protocolV2) messagePump(client *clientV2, startedChan chan bool) {
        ...
        for {
                ...
                case <-client.ReadyStateChan:
                        //客户端可以收的消息数变化时(比如RDY,FIN等), 则会向ReadyStateChan中写入消息，下面的case条件则可满足，重新进入for循环
                case subChannel = <-subEventChan:
                        // you can't SUB anymore
                        //接收到客户端发送的SUB命令后，会向subEventChan中写入消息，subEventChan则被置为nil，所以一个客户端只能订阅一次Channel
                        subEventChan = nil
                case identifyData := <-identifyEventChan:
                        // you can't IDENTIFY anymore, 和SUB  一样, 一个client 只能IDENTIFY 一次
                ...
                case <-heartbeatChan: // 心跳( nsqd 向 client 主动推送)
...
func (p *protocolV2) SUB(client *clientV2, params [][]byte) ([]byte, error) {
        ...
        // 对于临时channel/topic, 最后一个订阅的client关闭后,临时channel/topic 也会被关闭
        // 此处for 循环是为了防止订阅的临时channel 在 getChannel() 和 AddClient() 之间被关闭
        var channel *Channel
        for {
        ...
        // 从这儿也可以看到一个client 只能订阅一个channel, 如果订阅多个channel, 会出现消息混淆,
        // 因为订阅之后 nsqd 会给 client 推送message, 但message 不包含任何关于topic & channel 的信息
        // 1 是为了 设计更简单, 2 也是为了避免冗余传输
...
// client 收到 message, 但要求延迟发送(比如处理出现问题时,需要重试)
