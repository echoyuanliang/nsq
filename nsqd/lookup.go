...
func (n *NSQD) lookupLoop() {
        ...
        for {
                if connect {
                        for _, host := range n.getOpts().NSQLookupdTCPAddresses { // 连接lookupd 服务,  并广播topic/channel 信息至lookupd
                ...
                case <-ticker:
                        ...
                        for _, lookupPeer := range lookupPeers {
                                ...
                                cmd := nsq.Ping()
                                _, err := lookupPeer.Command(cmd) // command 执行出错时会自动释放连接
                                if err != nil {
                                        n.logf(LOG_ERROR, "LOOKUPD(%s): %s - %s", lookupPeer, cmd, err)
                                }
                        }
                case val := <-n.notifyChan: // 广播Topic、Chanel 信息变化至lookupd
                        var cmd *nsq.Command
                        var branch string
                ...
                case <-n.optsNotificationChan: // lookupd 配置时进行 同步变更
