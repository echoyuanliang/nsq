...
type clientV2 struct {
        ...
        ReadyCount    int64  // 客户端一次可接收的message 数量
        InFlightCount int64  // 已发送但未得到确认的Message数量
        MessageCount  uint64 // 所有已发送Message 数量
        FinishCount   uint64 // 已经确认发送成功的Message 数量
        RequeueCount  uint64 // 重新入队的Message 数量

        pubCounts map[string]uint64 // producer 在各个Topic发布的消息数量
