...
func (n *NSQD) resizePool(num int, workCh chan *Channel, responseCh chan bool, closeCh chan int) {
        ...
        for {
                if idealPoolSize == n.poolSize {
                        ...
                        // contract
                        closeCh <- 1 // 每次写入closeCh, 即会随机close 一个 worker, 同时减少worker pool 数量
                        n.poolSize--
                } else {
                        // expand
                        n.waitGroup.Wrap(func() {
...
func (n *NSQD) queueScanWorker(workCh chan *Channel, responseCh chan bool, closeCh chan int) {
        for {
                select {
                case c := <-workCh: // 从worker ch 中拿到一个待处理Channel
                        now := time.Now().UnixNano()
                        dirty := false
                        if c.processInFlightQueue(now) { // 处理已发送队列
                                dirty = true
                        }
...
func (n *NSQD) queueScanLoop() {
        ...
        loop:
                for _, i := range util.UniqRands(num, len(channels)) { // 随机抽取 QueueScanSelectionCount 个 channels 写入workCh进行处理
                        workCh <- channels[i]
                }

                numDirty := 0
                for i := 0; i < num; i++ { // 计算dirty 数量
                        if <-responseCh {
                                numDirty++
                        }
                }
                ...
                if float64(numDirty)/float64(num) > n.getOpts().QueueScanDirtyPercent {
                        // dirty 数量(仍然存在已发送/已延迟消息的Channel) > QueueScanDirtyPercent时 继续写入 workCh处理
                        // 否则 进入 select 查看是否到了定时器时间, 是否需要调整处理worker数量, 是否需要退出
                        goto loop
                }
        }
